...
bool FuzzIsLockedTest(const uint8_t *rawData, size_t size)
{
    uint32_t code = static_cast<uint32_t>(ScreenLockServerIpcInterfaceCode::IS_LOCKED);
    MessageParcel data;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::FuzzIsLockedTest(data, size);
