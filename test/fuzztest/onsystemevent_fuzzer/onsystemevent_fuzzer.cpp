...
#include "onsystemevent_fuzzer.h"
...
bool FuzzOnSystemEventTest(const uint8_t *rawData, size_t size)
{
    uint32_t code = static_cast<uint32_t>(ScreenLockServerIpcInterfaceCode::ONSYSTEMEVENT);
    MessageParcel data;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::FuzzOnSystemEventTest(data, size);
